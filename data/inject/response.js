  ...
  const run = em.run = async (oResult, mode = 'normal') => {
    ...
    try {
      if (lang !== 'detect') {
        ...
        await Promise.all([
          ...
          if (r.r.languages.length) {
            ...
            command('rename', `Tự động nhận diện (${e || r.lang})`);
            command('message', `Đã phát hiện ngôn ngữ "${e || r.lang}". Vui lòng chờ...`);
          }
          o = o || r.o;
        });
      }
      ...
      if (o.confidence < 50 && !oResult) {
        command('message', `Độ tin cậy thấp (${o.confidence}%). Đang tối ưu màu tương phản. Vui lòng chờ...`);
        await new Promise(resolve => setTimeout(resolve, 2000));
        run(o, 'invert');
        return;
      }
      if (mode === 'invert' && o.confidence < 20 && oResult.confidence < 20) {
        command('message', `Đang thử lại chế độ tối ưu ảnh. Vui lòng chờ...`);
      ...
      if (o.text.trim() === '') {
        command('build', '<span style="color: #ef4444; font-weight: 600;">⚠️ Không nhận diện được chữ trong vùng chọn! Hãy kéo thả ảnh hoặc chọn vùng khác rõ nét hơn.</span>');
      }
      else {
        command('build', o.hocr);
        command('enable');

        // Automatically save to InfoSys BANG_TAM
        try {
